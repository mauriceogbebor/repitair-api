 ...
 *
 * Boots the full Nest application context (no HTTP servers) so every domain
...
async function bootstrap() {
  ...
  if (!storageHealth.connected)
    throw new Error('Worker storage readiness probe failed');
  ...
  const environment =
    ...
    'unknown';
  const revision =
    config.get<string>('RAILWAY_GIT_COMMIT_SHA') ??
    config.get<string>('COMMIT_SHA') ??
    config.get<string>('SOURCE_VERSION') ??
    'unknown';
  const provider = normalizeProviderId(
    config.get<string>('BG_REMOVAL_PROVIDER'),
  );
  logger.log(
